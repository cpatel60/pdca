  ...
  public void startServer() {
	try{
   		...
   		while(true){
			...
			ParallelSocket socket1 = new ParallelSocket(socket, numConnections, this, cluster);
...
class ParallelSocket implements Runnable {
    ...
    public ParallelSocket(Socket socket, int id, Scheduler server, Cluster cluster) {
	this.socket = socket;
	this.id = id;
	this.server = server;
